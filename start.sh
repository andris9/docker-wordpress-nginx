...
ENDL
  ...
  echo mysql -h $DB_PORT_3306_TCP_ADDR -uroot -p$MYSQLPASS -e "DROP USER '$WORDPRESS_DB'@'localhost'; DROP DATABASE IF EXISTS $WORDPRESS_DB; CREATE DATABASE $WORDPRESS_DB; GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, DROP, INDEX ON $WORDPRESS_DB.* TO '$WORDPRESS_DB'@'localhost' IDENTIFIED BY '$MYSQL_USER_PASSWORD'; FLUSH PRIVILEGES;"
  # killall mysqld
fi
